...
import { SilentError, UploadSongError, isCloudinaryUrl, sleep } from "@newm.io/studio/common";
 ...
 */
export const uploadSong = createAsyncThunk("song/uploadSong", async (body: UploadSongRequest, { dispatch }) => {
  let songId = "";
  const progressDisclaimer = "Please do not refresh the page or navigate away while the " + "upload is in progress.";
  ...
  try {
    ...
    const barcodeType = body.barcodeNumber && body.barcodeType ? barcodeTypeMapping[body.barcodeType] : undefined;
    ...
    const songResp = await dispatch(
      songApi.endpoints.uploadSong.initiate({
        ...
        compositionCopyrightYear: body.compositionCopyrightYear || releaseYear,
        compositionCopyrightOwner: body.compositionCopyrightOwner || defaultCopyright,
        phonographicCopyrightYear: body.phonographicCopyrightYear || releaseYear,
        phonographicCopyrightOwner: body.phonographicCopyrightOwner || defaultCopyright,
        parentalAdvisory,
        barcodeType,
    ...
    if (body.isMinting) {
      const collaborators = generateCollaborators(body.owners, body.creditors, body.featured);
      ...
      dispatch(
        setProgressBarModal({
          progress: 95,
          message: "Requesting minting payment. " + "Please sign transaction when prompted.",
          disclaimer: progressDisclaimer,
          animationSeconds: 6,
        })
      );
    ...
    dispatch(
      setProgressBarModal({
        ...
        message: body.isMinting
          ? "Requesting minting payment. " + "Please sign transaction when prompted."
    ...
    if (songId) {
      try {
        await dispatch(songApi.endpoints.deleteSong.initiate({ songId, showToast: false }));
      } catch (error) {
        // do nothing
      }
    }
...
export const fetchSongStream = createAsyncThunk("song/streamSong", async (song: Song, { dispatch }) => {
  try {
    // get stream info for song
    const audioStreamResp = await dispatch(songApi.endpoints.getSongStream.initiate(song));
...
export const generateArtistAgreement = createAsyncThunk(
  ...
  async (body: GenerateArtistAgreementBody, { dispatch }) => {
    try {
      const artistAgreementResp = await dispatch(lambdaApi.endpoints.generateArtistAgreement.initiate(body));
 ...
 */
export const patchSong = createAsyncThunk("song/patchSong", async (body: PatchSongRequest, { dispatch }) => {
  try {
    ...
    if (body.coverArtUrl instanceof File) {
      ...
      coverArtUrl = await uploadToCloudinary(body.coverArtUrl as File, uploadParams, dispatch);
    ...
    if (body.owners?.length || body.creditors?.length || body.featured?.length) {
      await dispatch(
    ...
    if (body.isMinting) {
      const songResp = await dispatch(songApi.endpoints.getSong.initiate(body.id));

      if ("error" in songResp || !songResp.data) return;

      const profileResp = await dispatch(sessionApi.endpoints.getProfile.initiate());
...
export const getUserWalletSongs = createAsyncThunk(
  ...
  async (body: Omit<GetUserWalletSongsRequest, "utxoCborHexList">, { dispatch }) => {
    try {
 ...
 */
export const deleteSong = createAsyncThunk("song/deleteSong", async (body: DeleteSongRequest, { dispatch }) => {
  try {
 ...
 */
export const fetchInvites = createAsyncThunk("collaborator/fetchInvites", async (_, { dispatch }) => {
  ...
  if (!collaborationsData || !collaborationsData.length || "error" in collaborationsResponse) {
    return;
  }

  const collaboratorsPromises = collaborationsData.map((collaboration: Collaboration) =>
    createInvite(collaboration, dispatch)
  );
...
export const updateCollaborations = createAsyncThunk(
  ...
  async (body: UpdateCollaborationsRequest, { dispatch }) => {
    try {
      const currentCollabsResp = await dispatch(songApi.endpoints.getCollaborations.initiate({ songIds: body.id }));

      if ("error" in currentCollabsResp || !currentCollabsResp.data) return;

      const newCollaborators = generateCollaborators(body.owners || [], body.creditors || [], body.featured || []);

      const newCollabs = mapCollaboratorsToCollaborations(body.id, newCollaborators);

      const collabsToDelete = getCollaborationsToDelete(currentCollabsResp.data, newCollabs);

      const collabsToUpdate = getCollaborationsToUpdate(currentCollabsResp.data, newCollabs);

      const collabsToCreate = getCollaborationsToCreate(currentCollabsResp.data, newCollabs);
      ...
      const deleteCollabResponses = await Promise.all(
        collabsToDelete.map((collaborationId) => {
          return dispatch(songApi.endpoints.deleteCollaboration.initiate(collaborationId));
        })
      );
...
export const useGenerateArtistAgreementThunk = asThunkHook(generateArtistAgreement);
