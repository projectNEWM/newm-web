/**
 * NEWM External Links and Support
 */
export const NEWM_IO_URL = "https://newm.io";
export const NEWM_MARKETPLACE_FAQ_URL = "https://newm.io/faq";
export const NEWM_MARKETPLACE_SUPPORT_URL =
  "https://projectnewm.atlassian.net/servicedesk/customer/portal/1";
export const NEWM_MARKETPLACE_TERMS_OF_SERVICE_URL = "https://newm.io/tos";
export const NEWM_SUPPORT_EMAIL = "support@newm.io";
export const NEWM_TELEGRAM_URL = "https://t.me/NEWMofficial";
export const NEWM_X_URL = "https://twitter.com/projectNEWM";

/**
 * Map of grid column size for displaying artists based
 * on the screen size and item orientation.
 */
export const gridSizeColumnMap = {
  lg: { column: 2.4, row: 4 },
  md: { column: 3, row: 6 },
  sm: { column: 4, row: 6 },
  xs: { column: 6, row: 12 },
};
